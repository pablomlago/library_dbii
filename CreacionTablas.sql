...
(
  ...
  libro integer NOT NULL DEFAULT currval('seq_libro_id_libro'::regclass),
  ejemplar integer NOT NULL,
  fecha_prestamo TIMESTAMP NOT NULL DEFAULT now(),
  fecha_devolucion TIMESTAMP,
  CONSTRAINT prestamo_pkey PRIMARY KEY (usuario, libro, ejemplar, fecha_prestamo),
  CONSTRAINT prestamo_usuario_fkey FOREIGN KEY (usuario)
	REFERENCES usuario (id_usuario)
	ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT prestamo_ejemplar_fkey FOREIGN KEY (libro, ejemplar)
	REFERENCES ejemplar (libro, num_ejemplar)
	ON DELETE RESTRICT ON UPDATE CASCADE 
);
