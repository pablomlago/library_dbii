...
public class DAOLibros extends AbstractDAO {
    ...
    public Libro consultarLibroPrestamos(Integer idLibro){
        Libro resultado=null;
        Connection con;
        PreparedStatement stmLibro=null;
        ResultSet rsLibro;
        ...
        if (rsLibro.next())
        {
            ...
            try {
            stmEjemplares =con.prepareStatement("select ej.num_ejemplar, ej.localizador, ej.ano_compra, " +
                                                "pr.usuario, pr.fecha_prestamo::date, (pr.fecha_prestamo + INTERVAL '"+ DiasPrestamos.diasPrestamo + " days')::date as fecha_vencemento, " +
                                                //"coalesce(ej.num_ejemplar = pr.ejemplar, false) as foi_prestado " +
                                                "exists(select * from prestamo as pr2 where ej.num_ejemplar = pr2.ejemplar and ej.libro=pr2.libro) as foi_prestado " +
                                                "from ejemplar as ej left join prestamo as pr on ej.libro=pr.libro and ej.num_ejemplar=pr.ejemplar and fecha_devolucion is null " +
                                                "where ej.libro = ? ");
            System.out.println(stmEjemplares);
            stmEjemplares.setInt(1,resultado.getIdLibro());
            rsEjemplares=stmEjemplares.executeQuery();
            System.out.println(stmEjemplares);
            while (rsEjemplares.next())
            {
                 resultado.addEjemplar(new Ejemplar(resultado, rsEjemplares.getInt("num_ejemplar"),
                                                      rsEjemplares.getString("localizador"),
                                                      rsEjemplares.getString("ano_compra"),
                                                      rsEjemplares.getString("usuario"),
                         
                                                      rsEjemplares.getString("fecha_prestamo"),
                                                      rsEjemplares.getString("fecha_vencemento"),
                                                      rsEjemplares.getBoolean("foi_prestado")));
            }
            } catch (SQLException e){
                 System.out.println(e.getMessage());
                 this.getFachadaAplicacion().muestraExcepcion(e.getMessage());
    ...
    public java.util.List<Ejemplar> consultarEjemplaresLibroPrestamos(Integer idLibro){
        java.util.List<Ejemplar> resultado=new java.util.ArrayList<Ejemplar>();
        Connection con;
        PreparedStatement stmEjemplares=null;
        ResultSet rsEjemplares;
  ...
  public Boolean consultarLibrosCategoria(String nomeCategoria)
  {
    boolean resultado = false;
      
    Connection con;
    PreparedStatement stmLibros=null;
    ResultSet rsLibros;
    ...
    try  {
     stmLibros=con.prepareStatement("select libro " 
                                        + "from cat_tiene_libro as cl "
                                        + "where categoria = ? ");
     
    stmLibros.setString(1, nomeCategoria);
    rsLibros=stmLibros.executeQuery();
    
    resultado = rsLibros.next();

    } catch (SQLException e){
      System.out.println(e.getMessage());
      this.getFachadaAplicacion().muestraExcepcion(e.getMessage());
    }finally{
      try {stmLibros.close();} catch (SQLException e){System.out.println("Imposible cerrar cursores");}
    }
